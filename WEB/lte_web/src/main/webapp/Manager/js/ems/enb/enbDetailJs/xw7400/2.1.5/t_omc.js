$(function(){
			...
			$.ajax({
				...
				success:function(data){		
					...
					if(errorHtml != null && errorHtml != ""){					
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_OMC-2.1.5&isActiveEnbTable="+isActiveEnbTable+"";
					}				
				}
			});	
	...
	$("#t_omc tr").each(function(index){
		$("#t_omc tr:eq("+index+") td:eq(7)").click(function(){
			...
			var enbVersion = $("#enbVersion").val();
			var isActiveEnbTable = $("#isActiveEnbTable").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_OMC-2.1.5&referTable=T_IPV4&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
		});					   
	});	
	//跳转到配置
	$("#t_omc tr").each(function(index){
	...
	$("#t_omc tr").each(function(index){
		$("#t_omc tr:eq("+index+") td:eq(8)").click(function(){
			...
			if(confirm("确定要删除该条记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=delete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_OMC-2.1.5&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
			}
		});					   
	});	
	//批量删除
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("确定要删除所有选择的记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_OMC-2.1.5&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
			}
		}	
	});
	//取消按钮
