...
//		}
			...
			$.ajax({
				...
				success:function(data){	 
					...
					if(errorHtml != null && errorHtml != ""){					
						...
						window.location.href=""+basePath+"/lte/queryNbrcelRecords_xw7400.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_NBRCEL-3.0.8";
					}				
				}
			});	
		}
	...
	$("#t_cel_nbrcel1 tr").each(function(index){
		$("#t_cel_nbrcel1 tr:eq("+index+") td:eq(13)").click(function(){
			...
			var moId = $("#moId").val();
			var enbVersion = $("#enbVersion").val();
			window.location.href=""+basePath+"/lte/querySingleNbrcelRecord_xw7400.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_NBRCEL-3.0.8&parameters="+para+"";
		});					   
	});	
	//跳转到配置
	$("#t_cel_nbrcel1 tr").each(function(index){
