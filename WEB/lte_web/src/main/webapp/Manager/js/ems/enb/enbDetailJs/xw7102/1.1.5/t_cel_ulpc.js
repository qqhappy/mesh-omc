$(function(){
	...
	$("#fresh").click(function(){
		...
		window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ULPC-1.1.5";
	});
	//全选
	...
	$("#t_cel_ulpc1 tr").each(function(index){
		$("#t_cel_ulpc1 tr:eq("+index+") td:eq(15)").click(function(){
			...
			var moId = $("#moId").val();
			var enbVersion = $("#enbVersion").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ULPC-1.1.5&parameters="+para+"";
		});					   
	});	
	//跳转到配置
	$("#t_cel_ulpc1 tr").each(function(index){
	...
	$("#t_cel_ulpc1 tr").each(function(index){
		$("#t_cel_ulpc1 tr:eq("+index+") td:eq(16)").click(function(){
			...
			if(confirm("确定要删除该条记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=delete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ULPC-1.1.5&parameters="+para+"";
			}
		});					   
	});	
	//批量删除
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("确定要删除所有选择的记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ULPC-1.1.5&parameters="+para+"";
			}
		}	
	});
	//取消按钮
