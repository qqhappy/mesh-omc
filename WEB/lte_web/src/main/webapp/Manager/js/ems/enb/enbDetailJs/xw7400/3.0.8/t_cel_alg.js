$(function(){
	...
	$("#submit_add").click(function(){
		...
		if(!(isNum.test($("#u8DlMinMcs").val()) && $("#u8DlMinMcs").val()<=28  && $("#u8DlMinMcs").val()>=0)){
			...
			index++;
		}else if(parseInt($("#u8DlMinMcs").val()) > parseInt($("#u8DlMaxMcs").val())){
		...
		if(!(isNum.test($("#u8UlMinRbNum").val()) && $("#u8UlMinRbNum").val()<=100  && $("#u8UlMinRbNum").val()>=0)){
			...
			index++;
		}else if(parseInt($("#u8UlMinRbNum").val()) > parseInt($("#u8UlMaxRbNum").val())){
		...
		if(!(isNum.test($("#u8DlMinRbNum").val()) && $("#u8DlMinRbNum").val()<=100  && $("#u8DlMinRbNum").val()>=0)){
			...
			index++;
		}else if(parseInt($("#u8DlMinRbNum").val()) > parseInt($("#u8DlMaxRbNum").val())){
...
//		$("#au8UlCebBitmap").val(au8UlCebBitmap);
		...
		var para = "u8CId"+"="+$("#u8CId").val()+";"
				...
				+"u8UlMumimoEnable"+"="+$("input[name='u8UlMumimoEnable']:checked").val()+";"
				+"u8UlMumimoOrthThresh"+"="+accMul($("#u8UlMumimoOrthThresh").val(),100)+";"
				+"u8UlMumimoSinrThresh"+"="+$("#u8UlMumimoSinrThresh").val();
		$("#parameters").val(para);
		if(index==0){
			var moId = $("#moId").val();
			...
			$.ajax({
				...
				success:function(data){	
					...
					if(errorHtml != null && errorHtml != ""){					
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz_xw7400.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ALG-3.0.8";
					}				
				}
			});
	...
	$("#t_cel_alg tr").each(function(index){
		...
		if($("#t_cel_alg tr:eq("+index+") td:eq(24)").text() == 0){
			$("#t_cel_alg tr:eq("+index+") td:eq(24)").text("Elevel1");
		}else if($("#t_cel_alg tr:eq("+index+") td:eq(24)").text() == 1){
			$("#t_cel_alg tr:eq("+index+") td:eq(24)").text("Elevel2");
		}else if($("#t_cel_alg tr:eq("+index+") td:eq(24)").text() == 2){
			$("#t_cel_alg tr:eq("+index+") td:eq(24)").text("Elevel4");
		}else if($("#t_cel_alg tr:eq("+index+") td:eq(24)").text() == 3){
			$("#t_cel_alg tr:eq("+index+") td:eq(24)").text("Elevel8");
		}
		//
		...
		if($("#t_cel_alg tr:eq("+index+") td:eq(43)").text() == 0){
			$("#t_cel_alg tr:eq("+index+") td:eq(43)").text("Elevel4");
		}else{
			$("#t_cel_alg tr:eq("+index+") td:eq(43)").text("Elevel8");
		}
		//
	...
	$("#t_cel_alg td.u8UlMumimoEnable").each(function(){
	...
	//跳转到配置
	$("#t_cel_alg tr").each(function(index){
		$("#t_cel_alg tr:eq("+index+") td:eq(48)").click(function(){
			var u8CId = $("#t_cel_alg tr:eq("+index+") td:eq(0)").text();
			var para = "u8CId"+"="+u8CId;
			var basePath = $("#basePath").val();
			var moId = $("#moId").val();
			var enbVersion = $("#enbVersion").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz_xw7400.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ALG-3.0.8&referTable=T_CEL_PARA&parameters="+para+"";
		});					   
	});	
	//跳转到配置
	$("#t_cel_alg tr").each(function(index){
	...
	$("#t_cel_alg tr").each(function(index){
		$("#t_cel_alg tr:eq("+index+") td:eq(49)").click(function(){
			...
			if(confirm("确定要删除该条记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz_xw7400.do?enbVersion="+enbVersion+"&operType=delete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ALG-3.0.8&parameters="+para+"";
			}
		});					   
	});	
	//批量删除
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("确定要删除所有选择的记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz_xw7400.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_ALG-3.0.8&parameters="+para+"";
			}
		}	
	});
	//取消按钮
