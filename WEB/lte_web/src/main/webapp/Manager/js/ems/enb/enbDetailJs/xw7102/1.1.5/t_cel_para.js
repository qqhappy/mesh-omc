$(function(){
	...
	$("#submit_add").click(function(){
		...
		var au8DlAntPortMapLength = au8DlAntPortMap.length;
		if(au8DlAntPortMapLength < 8){
			for(var i = 0;i<8-au8DlAntPortMapLength;i++){
				au8DlAntPortMap = au8DlAntPortMap + "0";
			}
		}
		var index = 0;
		$(".error").text("");
		...
		if(!(isNum.test($("#u16AdmitMaxUeNum").val()) && $("#u16AdmitMaxUeNum").val()<=1500  && $("#u16AdmitMaxUeNum").val()>=1)){
			$("#u16AdmitMaxUeNumError").text("/* 请输入1~1500之间的整数 */");
			index++;
		}
		if(!(isNum.test($("#u16AdmitMaxErabNum").val()) && $("#u16AdmitMaxErabNum").val()<=2250  && $("#u16AdmitMaxErabNum").val()>=1)){
			$("#u16AdmitMaxErabNumError").text("/* 请输入1~2250之间的整数 */");
			index++;
		}
...
(isNum.test($("#au16AntSynWgtPhase8").val()) && $("#au16AntSynWgtPhase8").val()>=0 && $("#au16AntSynWgtPhase8").val()<=359 ))){
		...
		if($("#u8UeTransMode").val() != 0){
			...
			if($("#u8DlAntPortNum").val() != 0){
				index++;
				$("#u8DlAntPortNumError").text("/* 传输模式为tm1时,此项只可为port1 */");
			}
		}
			...
			$.ajax({
				...
				success:function(data){	
					...
					if(errorHtml != null && errorHtml != ""){
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_PARA-1.1.5";
					}				
				}
			});	
		}
	...
	// 跳转到配置
	$("#t_cel_para1 tr").each(function(index){
		$("#t_cel_para1 tr:eq("+index+") td:eq(49)").click(function(){
			var u8CId = $("#t_cel_para1 tr:eq("+index+") td:eq(0)").text();
			var para = "u8CId"+"="+u8CId;
			var basePath = $("#basePath").val();
			var moId = $("#moId").val();
			var enbVersion = $("#enbVersion").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_PARA-1.1.5&parameters="+para+"&referTable=T_ENB_MEASCFG&referTableTopo=T_TOPO";
		});					   
	});	
	// 跳转到配置
	$("#t_cel_para1 tr").each(function(index){
	...
	// 删除
	$("#t_cel_para1 tr").each(function(index){
		$("#t_cel_para1 tr:eq("+index+") td:eq(51)").click(function(){
			var u8CId = $("#t_cel_para1 tr:eq("+index+") td:eq(0)").text();
			var para = "u8CId"+"="+u8CId;
			var basePath = $("#basePath").val();
			var moId = $("#moId").val();
			if(confirm("与该小区相关的其他表记录也将删除，是否继续?")){
				...
				$.ajax({
					...
					success : function(data) {
						if(data.status == 0){
							window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select&tableName=T_CEL_PARA-1.1.5";
						}else{
							alert(data.error);
						}
					}
	...
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("与所选小区相关的其他表记录也将删除，是否继续?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_PARA-1.1.5&parameters="+para+"";
			}
		}	
	});
	// 取消按钮
