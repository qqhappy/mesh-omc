$(function(){
			...
			$.ajax({
				...
				success:function(data){		
					...
					if(errorHtml != null && errorHtml != ""){
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_BOARD-2.1.5&isActiveEnbTable="+isActiveEnbTable;
					}				
				}
			});						
		}
	...
	$("#t_board tr").each(function(index){
		$("#t_board tr:eq("+index+") td:eq(8)").click(function(){
			...
			var enbVersion = $("#enbVersion").val();
			var isActiveEnbTable = $("#isActiveEnbTable").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_BOARD-2.1.5&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
		});					   
	});	
	//跳转到配置
	$("#t_board tr").each(function(index){
