$(function(){
			...
			$.ajax({
				...
				success:function(data){		
					...
					if(errorHtml != null && errorHtml != ""){					
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_IPV4-1.1.5&isActiveEnbTable="+isActiveEnbTable+"";
					}				
				}
			});	
	...
	$("#t_ipv4 tr").each(function(index){
		//au8IPAddr//query
		var au8IPAddr = $("#t_ipv4 tr:eq("+index+") td:eq(2)").text().split("");	
		...
		$("#t_ipv4 tr:eq("+index+") td:eq(2)").text(result1);	
		
		var au8NetMask = $("#t_ipv4 tr:eq("+index+") td:eq(3)").text().split("");	
		...
		$("#t_ipv4 tr:eq("+index+") td:eq(3)").text(result2);	
		
		
		var au8GWAddrAttr = $("#t_ipv4 tr:eq("+index+") td:eq(4)").text();	
		...
		$("#t_ipv4 tr:eq("+index+") td:eq(4)").text(result3);
	});
	//刷新按钮
	...
	//跳转到配置
	$("#t_ipv4 tr").each(function(index){
		$("#t_ipv4 tr:eq("+index+") td:eq(5)").click(function(){
	...
	var enbVersion = $("#enbVersion").val();
			var isActiveEnbTable = $("#isActiveEnbTable").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_IPV4-1.1.5&referTable=T_ETHPARA&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
		});					   
	});	
	//跳转到配置
	$("#t_ipv4 tr").each(function(index){
	...
	$("#t_ipv4 tr").each(function(index){
		$("#t_ipv4 tr:eq("+index+") td:eq(6)").click(function(){
			...
			if(confirm("确定要删除该条记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=delete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_IPV4-1.1.5&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
			}
		});					   
	});	
	//批量删除
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("确定要删除所有选择的记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_IPV4-1.1.5&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
			}
		}		
	});
	//取消按钮
