$(function(){
	...
	$("#fresh").click(function(){
		...
		window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_SWPKG-1.1.5";
	});
	//全�?
...
//			var basePath = $("#basePath").val();
//			var moId = $("#moId").val();
//			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_SWPKG-1.1.5&parameters="+para+"";
//		});					   
//	});	
//	//跳转到配�?
//	$("#t_swpkg tr").each(function(index){
...
//			var moId = $("#moId").val();
//			if(confirm("确定要删除该条记�?")){
//				window.location.href=""+basePath+"/lte/queryEnbBiz.do?operType=delete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_SWPKG-1.1.5&parameters="+para+"";
//			}
//		});					   
//	});	
	//批量删除
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("确定要删除所有选择的记录?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_SWPKG-1.1.5&parameters="+para+"";
			}
		}	
	});
	//取消按钮
