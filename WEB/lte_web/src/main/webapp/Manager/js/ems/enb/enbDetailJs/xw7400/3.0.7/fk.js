$(function(){
	...
	//环境监控表和以太网表
	if(tableName == "T_ENVMON" || tableName == "T_ETHPARA" || tableName == "T_ETHPARA-3.0.7"){
		if($("input[name='operType']").val() == "config"){
			//判定机架号
...
function queryFk(moId,tableName,fkName,isMain){
	...
	$.ajax({
		...
		success:function(data){
			...
			//环境监控表和以太网表
			if(data.referFkModel.tableName == "T_ENVMON" || data.referFkModel.tableName == "T_ETHPARA"  || tableName == "T_ETHPARA-3.0.7"){
				if(data.referFkModel.fkName == "u8RackNO"){
					$("#u8RackNO").html(myOption);
				}
				if(data.referFkModel.fkName == "u8ShelfNO"){
