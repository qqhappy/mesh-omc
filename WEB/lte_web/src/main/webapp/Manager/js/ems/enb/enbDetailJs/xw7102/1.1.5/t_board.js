...
//		}	 	
			...
			$.ajax({
				...
				success:function(data){		
					...
					if(errorHtml != null && errorHtml != ""){
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_BOARD-1.1.5&isActiveEnbTable="+isActiveEnbTable;
					}				
				}
			});						
		}
	...
	$("#t_board tr").each(function(index){
		$("#t_board tr:eq("+index+") td:eq(8)").click(function(){
			var u8RackNO = $("#t_board tr:eq("+index+") td[info='u8RackNO']").attr("u8RackNO");
			...
			var enbVersion = $("#enbVersion").val();
			var isActiveEnbTable = $("#isActiveEnbTable").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_BOARD-1.1.5&parameters="+para+"&isActiveEnbTable="+isActiveEnbTable+"";
		});					   
	});	
	//跳转到配置
	$("#t_board tr").each(function(index){
	...
	$("#t_board tr").each(function(index){
		$("#t_board tr:eq("+index+") td:eq(9)").click(function(){
			if(confirm("确定进行复位?")){
				var rackId = $("#t_board tr:eq("+index+") td[info='u8RackNO']").attr("u8RackNO");
	...
	$("#delete").click(function(){
		...
		$("#t_board input[type=checkbox]").each(function(index){
			if($("#t_board input[type=checkbox]:eq("+index+")").attr("checked")){
				var temp1 = $("#t_board tr:eq("+index+") td[info='u8RackNO']").attr("u8RackNO");
