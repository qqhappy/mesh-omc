...
//设置频段代表的频段范围
function setFreqBandRange(u8FreqBandInd){	
	$("#u8FreqBandInd option").each(function(){
		var u8FreqBandInd = $(this).val();
		var UpperBoundary = getUpperBoundary(u8FreqBandInd);
		var LowerBoundary = getLowerBoundary(u8FreqBandInd);
		var optioHtml = $(this).val();	
		optioHtml = optioHtml+" ("+LowerBoundary+"~"+UpperBoundary+")(MHZ)";
		$(this).html(optioHtml);
	});
}
...
function cancelModify(){
	...
	var enbHexId= $("#enbHexId").val();
	if(enbVersion>="3.0.8"){
		window.location.href = basePath+"/lte/queryEnbBiz_xw7400.do?moId="+moId+"&operType=select&enbHexId="+enbHexId+"&enbVersion="+enbVersion+"&tableName=T_CEL_PARA-"+enbVersion;
	}else{
