...
(isNum.test($("#au16AntSynWgtPhase8").val()) && $("#au16AntSynWgtPhase8").val()>=0 && $("#au16AntSynWgtPhase8").val()<=359 ))){
			...
			$.ajax({
				...
				success:function(data){	
					...
					if(errorHtml != null && errorHtml != ""){
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_PARA-3.0.6";
					}				
				}
			});	
		}
	...
	$("#t_cel_para1 tr").each(function(index){
		$("#t_cel_para1 tr:eq("+index+") td:eq(46)").click(function(){
			...
			var moId = $("#moId").val();
			var enbVersion = $("#enbVersion").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_PARA-3.0.6&parameters="+para+"&referTable=T_ENB_MEASCFG&referTableTopo=T_TOPO";
		});					   
	});	
	// 跳转到配置
	$("#t_cel_para1 tr").each(function(index){
		...
		$("#t_cel_para1 tr:eq("+index+") td:eq(48)").click(function(){
			...
			if(confirm("与该小区相关的其他表记录也将删除，是否继续?")){
				...
				$.ajax({
					...
					success : function(data) {
						if(data.status == 0){
							window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select&tableName=T_CEL_PARA-3.0.6";
						}else{
							alert(data.error);
						}
					}
	...
	$("#delete").click(function(){
		...
		if(str.length < 1){
			...
			if(confirm("与所选小区相关的其他表记录也将删除，是否继续?")){
				var enbVersion = $("#enbVersion").val();
				window.location.href=""+basePath+"/lte/queryEnbBiz.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_CEL_PARA-3.0.6&parameters="+para+"";
			}
		}	
	});
	// 取消按钮
