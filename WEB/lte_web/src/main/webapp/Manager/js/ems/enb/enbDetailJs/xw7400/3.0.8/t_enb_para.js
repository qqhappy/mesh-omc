...
//		}
			...
			$.ajax({
				...
				success:function(data){		
					...
					if(errorHtml != null && errorHtml != ""){					
						...
						window.location.href=""+basePath+"/lte/queryEnbBiz_xw7400.do?enbVersion="+enbVersion+"&moId="+moId+"&operType=select"+"&browseTime="+getBrowseTime()+"&tableName=T_ENB_PARA-3.0.8&enbHexId="+enbHexId+"&enbName="+enbName+"&isActiveEnbTable="+isActiveEnbTable+"";
					}				
				}
			});	
		}
	...
	$("#t_enb_para tr").each(function(index){
		$("#t_enb_para tr:eq("+index+") td:eq(11)").click(function(){
			...
			var enbName = $("#enbName").val();
			var isActiveEnbTable = $("#isActiveEnbTable").val();
			window.location.href=""+basePath+"/lte/turnConfigEnbBiz_xw7400.do?moId="+moId+"&enbVersion="+enbVersion+""+"&browseTime="+getBrowseTime()+"&tableName=T_ENB_PARA-3.0.8&parameters="+para+"&enbHexId="+u32eNBId+"&enbName="+enbName+"&isActiveEnbTable="+isActiveEnbTable+"";
		});					   
	});	
	//跳转到配置
	$("#t_enb_para tr").each(function(index){
...
//			if(confirm("确定要删除该条记录?")){
				var enbVersion = $("#enbVersion").val();
//				window.location.href=""+basePath+"/lte/queryEnbBiz_xw7400.do?enbVersion="+enbVersion+"&operType=delete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_ENB_PARA-3.0.8&parameters="+para+"&enbHexId="+u32eNBId+"&enbName="+enbName+"";
//			}
			alert("此表不允许进行删除操作");
		});					   
	});	
...
//			if(confirm("确定要删除所有选择的记录?")){
				var enbVersion = $("#enbVersion").val();
//				window.location.href=""+basePath+"/lte/queryEnbBiz_xw7400.do?enbVersion="+enbVersion+"&operType=multiDelete&moId="+moId+""+"&browseTime="+getBrowseTime()+"&tableName=T_ENB_PARA-3.0.8&parameters="+para+"";
//			}
//		}	
		alert("此表不允许进行删除操作");
	});
