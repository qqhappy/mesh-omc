...
import com.xinwei.minas.server.micro.microapi.MicroDataInputInfo;
import com.xinwei.minas.server.micro.microapi.MicroEnbIputItem;
import com.xinwei.minas.server.micro.microapi.MicroEnbSceneOutput;
import com.xinwei.minas.server.micro.microapi.MicroEnbSceneRuleEngine;
...
public class MicroEnbCellServiceImpl implements MicroEnbCellService {
	
	private Log log = LogFactory.getLog(MicroEnbCellServiceImpl.class);
	...
	public void add(long moId, EnbCellStart enbCellStart) throws Exception {
		try {
			...
			// 通过开站参数获取生成的数据
			MicroEnbSceneOutput sceneOutput = getSceneOutput(enbCellStart);
	...
	public void validate(Enb enb, EnbCellStart enbCellStart) throws Exception {
		...
		// 在api中校验
		MicroDataInputInfo dataInputInfo = new MicroDataInputInfo();
		...
		dataInputInfo.setRruType(enbCellStart.getRruTypeId());
		MicroEnbSceneRuleEngine.checkEnbPara(1, dataInputInfo);
	 ...
	 * @throws Exception
	 */
	public MicroEnbSceneOutput getSceneOutput(EnbCellStart enbCellStart)
			throws Exception {
		List<MicroEnbIputItem> inputList = new ArrayList<MicroEnbIputItem>();
		// 封装输入参数
		inputList.add(new MicroEnbIputItem(EnbCellStart.ID_SCENE, enbCellStart
				.getSceneId()));
		inputList.add(new MicroEnbIputItem(EnbCellStart.ID_FREQ_BAND, enbCellStart
				.getFreqBandId()));
		inputList.add(new MicroEnbIputItem(EnbCellStart.ID_SYS_BAND_WIDTH,
				enbCellStart.getBandwidthId()));
		inputList.add(new MicroEnbIputItem(EnbCellStart.ID_SF_CFG, enbCellStart
				.getSfCfgId()));
		inputList.add(new MicroEnbIputItem(EnbCellStart.ID_RRU_TYPE, enbCellStart
				.getRruTypeId()));
		inputList.add(new MicroEnbIputItem(EnbCellStart.ID_AN_NUM, enbCellStart
				.getAnNumId()));
		// 调用API获取输出数据
		MicroEnbSceneOutput output = MicroEnbSceneRuleEngine.generate(1, inputList);
		return output;
	}
