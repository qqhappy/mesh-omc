...
public class XEnbBizConfigServiceImpl implements XEnbBizConfigService {

	private Log log = LogFactory.getLog(XEnbBizConfigServiceImpl.class);
	...
	private void compareAndSyncData(long moId, boolean emsToNe)
		...
		// 如果是网元到网管，则不同步ftp表数据
		topoTableNames.remove("T_FTP_PARA");
...
//			}
			...
			// 更新pci,rsi缓存
			//EnbBizUniqueIdHelper.postUpdate(moId, tableName, bizRecord);
			// 如果修改的是IPV4表中被OMC表引用的数据,则更新enb缓存中的IP、网关、掩码
			if(tableName.equals(EnbConstantUtils.TABLE_NAME_T_IPV4)) {
				XBizTable omcTable = enbBizConfigDAO.query(moId, EnbConstantUtils.TABLE_NAME_T_OMC, null);
				...
				String ipId = bizRecord.getStringValue(EnbConstantUtils.FIELD_NAME_IP_ID);
				if(enbIpIdField.getValue().equals(ipId)) {
					String ipAddress = bizRecord.getStringValue(EnbConstantUtils.FIELD_NAME_IP_ADDR);
					String netMask = bizRecord.getStringValue(EnbConstantUtils.FIELD_NAME_NET_MASK);
					String gateWay = bizRecord.getStringValue(EnbConstantUtils.FIELD_NAME_GATEWAY);
					enb.setPrivateIp(ipAddress);
