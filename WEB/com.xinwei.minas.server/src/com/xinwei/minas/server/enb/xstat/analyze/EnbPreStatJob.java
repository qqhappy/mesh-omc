 ...
 * ------------------------------------------------------------------------------
 * Date    	|  Who  		|  What  
 * 2014-8-10	| fanhaoyu 	| 	create the file                       
 */
...
public class EnbPreStatJob extends QuartzJobBean {

	private Log log = LogFactory.getLog(EnbPreStatJob.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		try {
			...
			log.error("EnbPreStatJob execute with error. startTime="
					+ this.startTime + ", endTime=" + this.endTime, e);
		}
	...
	private void calStartEndTime() {
		...
		if (this.preStatType == StatConstants.PRE_STAT_TYPE_HOUR) {
			...
			// 上一小时的起始时间
			this.startTime = (currentHourCount - 1) * oneHourMillis + 1000 * 15 * 60;
			// 上一小时的结束时间
			this.endTime = this.startTime + oneHourMillis;
			...
			// 上一天的起始时间
			this.startTime = (currentDayCount - 1) * oneDayMillis + 1000 * 15 * 60;
			// 上一天的结束时间
			this.endTime = this.startTime + oneDayMillis;
	 ...
	 *            预统计类型，按小时1或按天2
	 */
	public void setPreStatType(int preStatType) {
