...
public class EnbBizConfigService implements XMoBizConfigService {

	private Log log = LogFactory.getLog(EnbBizConfigService.class);
...
//			}
			...
			// 更新pci,rsi缓存
			//EnbBizUniqueIdHelper.postUpdate(moId, tableName, bizRecord);
			// 如果修改的小区参数表,需要处理关联到邻区表的数据
			if(tableName.equals(EnbConstantUtils.TABLE_NAME_T_CELL_PARA)) {
				enbNeighbourService.syncCellConfigToAll(moId, bizRecord);
			}
